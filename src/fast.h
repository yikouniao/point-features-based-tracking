...
class FastFeatureDetector {
 public:
  ...
  void detect(const Mat& img, std::vector<KeyPoint>& keypoints, const Mat& mask);
  int threshold;
...
class MaskPredicate {
public:
  MaskPredicate(const Mat& mask_) : mask(mask_) {}
...
void FAST(const Mat& _img, std::vector<KeyPoint>& keypoints, int threshold, bool non_max_suppression);
